...
private:
    ...
    CalibrationData calibration[USER_PROFILE_COUNT];
    int current_profile;
    GrindMode current_grind_mode;
    Preferences* preferences;

    void load_calibration();
    void save_calibration(int index);
...
public:
    ...
    float get_calibrated_flow_rate(int index) const;
    uint16_t get_calibration_count(int index) const;
    float get_calibrated_time(int index) const;  // weight / flow_rate
    void update_calibration(int index, float measured_flow_rate);
    void reset_calibration(int index);
};
